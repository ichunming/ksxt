...
public class ExcelUtil {
	 ...
	 */
	public static <T> List<T> convertToList(Class<T> clz) throws ExcelException {
	 ...
	 * @throws InvocationTargetException
	 */
	public static <T> List<T> convertToList(Class<T> clz, int rowFrom, int size) throws ExcelException {
		List<T> result = null; // result
		T obj = null; // object
		int rowTo = Math.min(rowFrom + size, sheet.getLastRowNum() + 1);
	 ...
	 * @throws IllegalArgumentException 
	 */
	public static <T> T convertToObj(Class<T> clz, int rownum) throws ExcelException {
		T obj = null; // object
		...
		try {
			...
			logger.error("fail to convert excel record to object.");
			e.printStackTrace();
